  ...
  <body>
      ...
      <form method="post" action="/phpmotors/vehicles/index.php">
        <div class="register">
          ...
          <div class="clientMake">
            ...
            <input type="text" id="clientMake" name="clientMake" placeholder="Honda"
                   required <?php if (isset($clientMake)) {
              echo "Value='$clientMake'";
            } ?>> <br>
          </div>
          <div class="clientModel">
            <label for="clientModel">Model:</label><br>
            <input type="text" id="clientModel" name="clientModel" placeholder="Civic"
                   required <?php if (isset($clientModel)) {
              echo "Value='$clientModel'";
            } ?>> <br>
          </div>
          <div class="clientDescription">
            <label for="clientDescription">Description:</label><br>
            <textarea id="clientDescription" name="clientDescription" placeholder="Enter a description for the vehicle"
                      required <?php if (isset($clientDescription)) {
              echo "Value='$clientDescription'";
            } ?>></textarea> <br>
          </div>
          <div class="clientImagePath">
            <label for="clientImagePath">Image Path:</label><br>
            <input type="text" id="clientImagePath" name="clientImagePath" value="/phpmotors/images/no-image.png"
                   required <?php if (isset($clientImagePath)) {
              echo "Value='$clientImagePath'";
            } ?>> <br>
          </div>
          <div class="clientThumbnailPath">
            <label for="clientThumbnailPath">Thumbnail Path:</label><br>
            <input type="text" id="clientThumbnailPath" name="clientThumbnailPath"
                   value="/phpmotors/images/no-image.png" required <?php if (isset($clientThumbnailPath)) {
              echo "Value='$clientThumbnailPath'";
            } ?>> <br>
          </div>
          <div class="clientPrice">
            <label for="clientPrice">Vehicle Price</label><br>
            <input type="number" id="clientPrice" name="clientPrice" placeholder="1000"
                   required <?php if (isset($clientPrice)) {
              echo "Value='$clientPrice'";
            } ?>> <br>
          </div>
          <div class="clientColor">
            <label for="clientColor">Vehicle Color</label><br>
            <input type="text" id="clientColor" name="clientColor" placeholder="black"
                   required <?php if (isset($clientColor)) {
              echo "Value='$clientColor'";
            } ?>> <br>
          </div>
          <div class="clientStock">
            <label for="clientStock">Vehicle Stock</label><br>
            <input type="number" id="clientStock" name="clientStock" placeholder="1"
                   required <?php if (isset($clientStock)) {
    ...
    <footer>
      ...
      <div>
        ...
        <p>
          All images used are believed to be in "Fair Use". Please notify the author if they are not and they will be
          removed.
        </p>
        <p>
